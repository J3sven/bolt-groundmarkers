...
function M.hookSwapBuffers(state, bolt, surfaces, colors)
    bolt.onswapbuffers(function(event)
        ...
        -- Get player position and chunk coordinates
        ...
        local _, playerChunkX, playerChunkZ, _, _ = coords.tileToRS(tileX, tileZ, py)
        
        local gx, gy, gw, gh = bolt.gameviewxywh()
        
        -- Filter tiles to include those within 1 chunk of the player (3x3 chunk area)
        local chunkTiles = {}
        for _, t in pairs(marked) do
            local chunkDiffX = math.abs(t.chunkX - playerChunkX)
            local chunkDiffZ = math.abs(t.chunkZ - playerChunkZ)
            if chunkDiffX <= 1 and chunkDiffZ <= 1 then
                chunkTiles[#chunkTiles + 1] = t
            end
        end
        
        -- Group filtered tiles by color
        local byColor = {}
            ...
            if coloredSurface then
                for _, t in ipairs(list) do
                    ...
                    for i, corner in ipairs(corners3D) do
                        ...
                        if depth <= 0.0 or depth > 1.0 then 
                            allVisible = false
                            break 
                        end
